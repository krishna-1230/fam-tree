...
func SearchPersons(c *gin.Context) {
	...
	cursor, err := collection.Find(ctx, bson.M{
		"name": bson.M{"$regex": primitive.Regex{Pattern: regexp.QuoteMeta(q), Options: "i"}},
	})
	if err != nil {
...
type relEntry struct {
	...
	relType  string
	source   models.RelationshipSource
}
...
func GetPerson(c *gin.Context) {
	...
	for _, rel := range rels {
		source := relationshipSource(rel)
		if rel.FromPersonID == personID {
			switch rel.Type {
			case "father", "mother":
				childEntries = append(childEntries, relEntry{relID: rel.ID, personID: rel.ToPersonID, relType: rel.Type, source: source})
			case "spouse":
				spouseEntries = append(spouseEntries, relEntry{relID: rel.ID, personID: rel.ToPersonID, relType: rel.Type, source: source})
			case "sibling":
				siblingEntries = append(siblingEntries, relEntry{relID: rel.ID, personID: rel.ToPersonID, relType: rel.Type, source: source})
			}
		} else {
			switch rel.Type {
			case "father", "mother":
				parentEntries = append(parentEntries, relEntry{relID: rel.ID, personID: rel.FromPersonID, relType: rel.Type, source: source})
			case "spouse":
				spouseEntries = append(spouseEntries, relEntry{relID: rel.ID, personID: rel.FromPersonID, relType: rel.Type, source: source})
			case "sibling":
				siblingEntries = append(siblingEntries, relEntry{relID: rel.ID, personID: rel.FromPersonID, relType: rel.Type, source: source})
			}
		}
	}
	...
	for key, value := range body {
		...
		if key == "date_of_birth" {
			if dateString, ok := value.(string); ok && dateString != "" {
				if parsed, parseErr := time.Parse(time.RFC3339, dateString); parseErr == nil {
					set[key] = parsed
					continue
				}
				if parsed, parseErr := time.Parse("2006-01-02", dateString); parseErr == nil {
					set[key] = parsed
					continue
				}
			}
...
func DeletePerson(c *gin.Context) {
	...
	if err = rebuildInferredRelationships(ctx); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Person deleted, but relationship graph could not be rebuilt"})
		return
	}
...
func fetchRelatedPersons(ctx context.Context, collection *mongo.Collection, entries []relEntry) []models.RelatedPerson {
	...
	relIDMap := make(map[primitive.ObjectID]relEntry, len(entries))
	for index, entry := range entries {
		ids[index] = entry.personID
		relIDMap[entry.personID] = entry
	}
	...
	for _, person := range persons {
		entry := relIDMap[person.ID]
		result = append(result, models.RelatedPerson{
			Person:             person,
			RelationshipID:     entry.relID,
			RelationshipType:   entry.relType,
